...
const MetadataForm = ({ setMetaData, metadata }: MetadataFormProps) => {
  console.log(metadata)
  const debouncedFetch = useDebounceCallback( (term: string, value: string) => {
        ...
        <form style={{ width: '100%' }}>
          <Grid container spacing={2}>
            {dublinCoreSample.map((field) => (
              <Grid item xs={12} key={field.term}>
                <Grid container alignItems="center" spacing={2}>
                  <Grid item xs={11}>
                    <TextField
                      ...
                      fullWidth
                      variant="outlined"
                      value={metadata[field.term]}
                      onChange={(e) => debouncedFetch(field.term, e.target.value)}
                      inputProps={{
                        style: {
                          whiteSpace: 'nowrap',
                          overflow: 'hidden',
