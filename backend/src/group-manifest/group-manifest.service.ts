...
export class GroupManifestService {
  ...
  async create(createGroupManifestDto: CreateGroupManifestDto) {
    try {
      ...
      const manifest = await this.manifestService.create(createGroupManifestDto)
      ...
      console.log(error);
      throw new InternalServerErrorException(`An error occurred while creating manifest, ${error.message}`);
    }
  }
  ...
  async addManifestToGroup(AddManifestToGroupDto: AddManifestToGroupDto){
    ...
    return manifestsForGroup;
      ...
      throw new InternalServerErrorException(`An Error occurred while adding manifests to userGroup with id ${userGroup.id} : ${error.message}`);
    }
  }

  async getAllManifestsGroup(manifestId: number) {
    try{
    return await this.linkGroupManifestService.findAllUserGroupByManifestId(manifestId);
    }catch(error){
      console.log(error)
      throw new InternalServerErrorException('an error occurred while getting all manifests group', error.message);
    }
  }

  async getManifestForUser(userGroupId: number, manifestId: number){
    try{
      const manifest = await this.linkGroupManifestService.findAllManifestGroupByUserGroupId(
        userGroupId,
      );
      return manifest.find((linkGroupManifest) => linkGroupManifest.manifest.id == manifestId);

    }catch(error){
      return new InternalServerErrorException('An error occurred while getting manifest for user group', error.message);
    }
  }
}
